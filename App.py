...
def main():
    ...
    CreditScore=Module.credit_score(st.number_input('Credit Score', min_value=0, max_value=1000, value=400, step=50))
    Age = Module.Age(st.number_input('Age', min_value=18, max_value=100, value=30, step=1))
    Tenure =st.number_input('Tenure', min_value=1, max_value=20, value=5 ,step=1)
    Balance =st.number_input('Balance', min_value=1, max_value=300000, value=50000 ,step=1000)
    NumOfProducts =st.number_input('NumOfProducts', min_value=0, max_value=10, value=5 ,step=1)
    EstimatedSalary = st.number_input('EstimatedSalary', min_value=0, max_value=300000, value=100000 ,step=1000)
    
    
    
    card_selection=["Yes","No"]
    
    HasCrCard =Module.Question(st.selectbox('HasCrCard', card_selection))
    
    IsActiveMember =Module.Question(st.selectbox('IsActiveMember', card_selection))
    ...
    geo=st.selectbox('Geography', geo_selection)
